...
<div class='col-xs-12'>    
    ...
    <form action="" method="post">
                    ...
                    <tr><td>
                        <div class='form-check form-check-flat form-check-primary'>
                            <label class='form-check-label'>
                                <input class="form-check-input" type='checkbox' <?=$attendance_type?> value='<?=$row->its_id?>' name='family_its_list[]' id='family_its_list[]'>
                            </label>
                        </div>
                    </td>
                    <td><div class='form-check form-check-flat form-check-primary'><label class='form-check-label'><input class="form-check-input" type='checkbox' <?=$chair_preference?> value='<?=$row->its_id?>'
                name='chair_its_list[]' id='chair_its_list[]'></label></div></td>
                    <!--
        ...
        <div class="mb-3 row">
            ...
            <div class="col-sm-9">
                <div class='form-check form-check-flat form-check-primary'>                            
                <label class='form-check-label'><input class='form-check-input' type='checkbox' name='pirsa' value='Y' <?=$dao->pirsa_selected?> ></label>
            </div>
                <!-- <input class="form-check-input" <?=$dao->pirsa_selected?> type='checkbox' value='Y'
                name='pirsa' id='pirsa'> -->
            </div>
        </div>
        <div class="mb-3 row" id="pirsa_comment_row" style="display: <?= (isset($dao->pirsa_selected) && trim($dao->pirsa_selected)<>'' )? 'block' : 'none' ?>;">
            <label for="pirsa_comment" class="col-sm-3 col-form-label">Pirsa comment / reason</label>
            <div class="col-sm-9">
                <textarea required name="pirsa_comment" id="pirsa_comment" class="form-control" rows="3"><?= isset($dao->pirsa_comment) ? htmlspecialchars($dao->pirsa_comment) : '' ?></textarea>
            </div>
        </div>
