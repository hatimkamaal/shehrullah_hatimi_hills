...
if ( !(is_user_role(SUPER_ADMIN) || is_user_role(FINANCE))) {
    do_redirect_with_message('/home', 'Unauthorized access.');
}
...
function __handle_post()
{
    ...
    if ($action === 'register') {
        ...
        $sehori_count = $_POST['sehori_count'];
...
function content_display()
{
    ...
    <form method="post">
        ...
        <div class='col-xs-12'>
            ...
            <div class="mb-3 row">
                <label for="hof_id" class="col-sm-3 col-form-label">Sehori count (<?= $hub_data->sehori ?>)</label>
                <div class="col-sm-9">
                    <select class="form-control form-control-lg" name="sehori_count" id="sehori_count">
                        <?= getCountDropdown(0, 5, $takhmeen_data->sehori_count ?? 0) ?>
                    </select>
                </div>
            </div>
            <div class="mb-3 row">
            ...
            <div class="form-group" style="font-weight:20px;margin-top: 25px;">
                ...
                <?php if ($takhmeen_data->takhmeen > 0) { ?>
                    <a id='paynow_link' href="get2post?url=/finance.collection&hof_id=<?= $hof_id ?>" class="btn btn-primary">Pay</a>
                <?php } ?>
            </div>
    ...
    <script>
        function on_change_dropdown() {
            ...
            var sehori_hub = Number($('#sehori_count').val()) * <?= $hub_data->sehori ?>;
            var pirsa_hub = Number($('#pirsa_count').val()) * <?= $hub_data->pirsu ?>;            
            var chair_hub = Number($('#chair_count').val()) * <?= $hub_data->chair ?>;

            var takhmeen = niyaz_hub + iftar_hub + zabihat_hub + fateha_hub + khajoor_hub + pirsa_hub + chair_hub + sehori_hub;
